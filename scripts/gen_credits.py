...
def _extra_marker(req: Requirement) -> str | None:
    if not req.marker:
        return None
    try:
        return next(marker[2].value for marker in req.marker._markers if getattr(marker[0], "value", None) == "extra")  # type: ignore[union-attr,unused-ignore]
...
def _get_deps(base_deps: dict[str, Requirement], metadata: Metadata) -> Metadata:
    ...
    for dep_name, dep_req in base_deps.items():
        if dep_name not in metadata or dep_name == "mkdocstrings-go":
            continue
