...
def check_api(ctx: Context, *cli_args: str) -> None:
    ...
    ctx.run(
        tools.griffe.check("mkdocstrings_handlers.go", search=["src"], color=True).add_args(*cli_args),
...
@duty
def build_go(ctx: Context, goos: str = "", goarch: str = "") -> None:
    """Build the Go shared library for the target platform.

    Parameters:
        goos: Target OS (default: current OS). E.g. linux, darwin, windows.
        goarch: Target architecture (default: current arch). E.g. amd64, arm64.
    """
    _goos = goos or _GOOS_MAP.get(platform.system(), platform.system().lower())
    _goarch = goarch or _GOARCH_MAP.get(platform.machine(), platform.machine().lower())
    _ext = _EXT_MAP.get(_goos, "so")
    ...
    (_GO_OUT_DIR / "libcollector.h").unlink(missing_ok=True)
