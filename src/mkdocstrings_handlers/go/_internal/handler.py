...
from mkdocstrings_handlers.go._internal.collector import _collect as gocollect
from mkdocstrings_handlers.go._internal.collector import _GoObject as GoObject
...
def _find_import_path(root: Path) -> str:
    """Find the Go project import path and root directory.

    Parameters:
        root: The starting directory to search for the Go project.
    """
    # Look for go.mod
    go_mod = root / "go.mod"
    if go_mod.is_file():
    ...
    raise FileNotFoundError(f"go.mod not found in {root}")
...
class GoHandler(BaseHandler):
    ...
    def __init__(self, config: GoConfig, base_dir: Path, **kwargs: Any) -> None:
        ...
        self._collected: dict[str, GoObject] = {}
    ...
    def collect(self, identifier: str, options: GoOptions) -> GoObject:
        ...
        root = options.go_project_root or self.base_dir
        import_path = _find_import_path(root)

        # item = gocollect(import_path, f"{root.absolute()}", identifier)

        self._collected[identifier] = gocollect(import_path, f"{root.absolute()}", identifier)
        return self._collected[identifier]

        # raise CollectionError("Implement me!")

    def render(self, data: GoObject, options: GoOptions) -> str:  # type: ignore[override,unused-ignore]
