...
if TYPE_CHECKING:
    ...
    from mkdocstrings_handlers.go.handler import GoHandler
...
def fixture_mkdocs_conf(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[config.Config]:
    ...
    Yields:
        MkDocs config.
    """
    conf = config.Config(schema=get_schema())  # type: ignore[call-arg, unused-ignore]
    ...
    conf_dict = {
        ...
        "plugins": [{"mkdocstrings": {"default_handler": "go"}}],
